...
Sub Initialise()
    RetryCount = 5
    Sec_10 = 20
    StdReadStartAddress = 2000       'D100
    StdReadCount = 100            'D100-D149
    StdWriteStartAddress = 2100     'D200
...
End Sub
Public Function PrintLabel(lPrinter As JustPrinter, Barcodeprint As String)
On Error GoTo err1
...
Dim TempStartingString As String
    ...
    TempPartNo = Mid(Barcodeprint, 1, 2)
    tempdate = Mid(Barcodeprint, 3, 5)
    Counter = Mid(Barcodeprint, 8, 4)
    PrnFile = "switch.prn"
    'barcode = PartNo & Counter & TempDate
    TempFileTextLine = ReadLabel(App.Path & "\PrnFiles\" & PrnFile)
    TempFileTextLine = Replace(TempFileTextLine, "#AS02K230001#", Barcodeprint)
    TempFileTextLine = Replace(TempFileTextLine, "#AS#", TempPartNo)
    TempFileTextLine = Replace(TempFileTextLine, "#02K23#", tempdate)
    TempFileTextLine = Replace(TempFileTextLine, "#0001#", Counter)
...
Dim tempdate As String
 tempdate = Format(DateDiff("d", DateSerial(year(Now), 1, 1), Now) + 1, "000") & Format(Now, "yy")
 Counter = Format(frmMonitor.txtProductionCounter.Text, "0000")
 barcode = AddSpacesToReachLength(PartNo, 15) & VendorCode & tempdate & Counter & Format(RevisionNo, "0")
        ...
        Dim bRet As Boolean
        bRet = ObjDoc.Open(App.Path & "/prnfiles/print.lbx")
        
        If (bRet <> False) Then
            'Assign data to the text object of "Fixed asset name"
            ObjDoc.GetObject("Data1").Text = PartNo
            
            'Assign data to the text object of "Management section"
            ObjDoc.GetObject("Data2").Text = VendorCode & ":" & Counter
            
            'Assign data to the text object of "Identification No."
            ObjDoc.GetObject("Data3").Text = tempdate
            
            'Assign data to the barcode object
            ObjDoc.GetObject("Barcode").Text = barcode
            ...
            If FSO.FileExists(App.Path & "/prnfiles/temp.bmp") = True Then
                FSO.DeleteFile App.Path & "/prnfiles/temp.bmp", True
            End If
            ObjDoc.SaveAs bexBmp, App.Path & "/prnfiles/temp.bmp"
...
Private Function AddSpacesToReachLength(inputString As String, targetLength As Integer) As String
    ...
    If currentLength < targetLength Then
        ...
        spacesToAdd = targetLength - currentLength
        
        ' Add spaces to the end of the string
        inputString = inputString & Space$(spacesToAdd)
    End If
    
    ' Return the modified string
    AddSpacesToReachLength = inputString
End Function
