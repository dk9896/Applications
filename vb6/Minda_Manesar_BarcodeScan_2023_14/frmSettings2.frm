...
If MsgBox("Do you want to save Coupler Counter", vbYesNo) = vbYes Then
    ...
    Set rs = New ADODB.Recordset
    rs.Open Sql, Con, adOpenDynamic, adLockOptimistic
    rs("CouplerCounter") = Val(txtSaveCoupler(Index).Text)
    rs.Update
End If
End Sub
...
If MsgBox("Do you want to set Coupler counter", vbYesNo) = vbYes Then
 SaveSetting App.Title, ModelName, "CouplerCounter", Val(txtSetCoupler(Index).Text)
End If
End Sub

Private Sub cmdsetNG_Click(Index As Integer)
If MsgBox("Do you want to set NG counter", vbYesNo) = vbYes Then
 SaveSetting App.Title, ModelName, "NGCounter", Val(txtngcount(Index).Text)
End If
End Sub
...
Private Sub cmdsetOK_Click(Index As Integer)
    If MsgBox("Do you want to set OK counter", vbYesNo) = vbYes Then
        SaveSetting App.Title, ModelName, "OkCounter", Val(txtokcount(Index).Text)
    End If
End Sub
...
Dim Sql As String
    ...
    txtSaveCoupler(0).Text = Val(rs("CouplerCounter"))
    txtProduction.Text = rs("productioncounter")
    txtokcount(0).Text = Val(GetSetting(App.Title, ModelName, "OkCounter", 0))
    txtngcount(0).Text = Val(GetSetting(App.Title, ModelName, "NgCounter", 0))
    txtSetCoupler(0).Text = Val(GetSetting(App.Title, ModelName, "CouplerCounter", 0))
